## Hi there! 👋 I'm Shahwaiz Javaid.

I'm a Frontend Developer dedicated to creating responsive, high-performance, and visually engaging user interfaces. I specialize in translating design layouts into modular, pixel-perfect React & Next.js applications, with a strong focus on clean code and fluid usability.

🔗 You can explore my full work history on my [Portfolio Website](https://shahwaizjavaid.vercel.app/) or connect with me on [LinkedIn](https://www.linkedin.com/in/shahwaiz-javaid).

---

### 🛠️ Core Technologies & Tools

* **Frontend & UI:** React.js, Next.js, JavaScript (ES6+), Tailwind CSS, Framer Motion, HTML5, CSS3
* **Workflows & Data:** TypeScript, Git, GitHub, Vercel, Supabase (Basic)

---

### 📁 Selected Projects & Client Work

* 🎙️ **[GoVoice.ai](https://govoice.ai/)** – An AI voice agent platform designed to automate outbound and inbound calls for businesses.
* ✍️ **[AutoBlogging Pro](https://www.autoblogging.pro/)** – An automated article writing and publishing platform that generates SEO blog content.
* 🎬 **[Videohomes](https://www.videohomes.com/)** – A video-hosting marketplace connecting real estate agents with professional videographers.
* 🏠 **[Las Hawaii Mansions](https://las-hawaii-mansions.vercel.app/)** – A luxury vacation rental platform showcasing themed mansions in Las Vegas and Orlando.

---

### ✉️ Let's get in touch

If you want to collaborate on a project or just talk frontend development, feel free to reach out via email at [shahwaizjavaid192@gmail.com](mailto:shahwaizjavaid192@gmail.com).
